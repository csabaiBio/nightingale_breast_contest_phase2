...
class HIPT_LGP_FC_STAGE3ONLY(nn.Module):
    ...
    def __init__(self, dropout=0.25, n_classes=5):
        ...
        self.global_phi = nn.Sequential(nn.Linear(size, size), nn.ReLU(), nn.Dropout(dropout))
        ...
        self.classifier_metastatic = nn.Linear(size, 1)
    ...
    def forward(self, h_4096, **kwargs):
        ...
        metastatic = self.classifier_metastatic(h_WSI)
        metastatic = self.sigmoid(metastatic)
        #print('IN MODEL:', logits, metastatic)
        
        return logits, F.softmax(logits, dim=1), Y_hat, None, None, metastatic
