...
class Attn_Net_Gated(nn.Module):  ### original D: 256
    def __init__(self, L = 384, D = 256, dropout = False, n_classes = 1):
        ...
        if dropout:
            ########################################## original was 0.25
            self.attention_a.append(nn.Dropout(0.6))
            self.attention_b.append(nn.Dropout(0.6))
...
class HIPT_LGP_FC_STAGE3ONLY(nn.Module):
    ...
    def __init__(self, dropout=0.25, n_classes=5):
        ...
        self.global_rho = nn.Sequential(*[nn.Linear(size, size), nn.ReLU(), nn.Dropout(dropout)])
